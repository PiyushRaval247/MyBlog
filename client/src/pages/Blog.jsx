...
const Blog = () => {
  ...
  const fetchBlogData = async () => {
   ...
   const fetchComments = async () => {
       try{
         const {data} = await axios.post(`${BASE_URL}/api/blog/comments`, {blogId : id})
         if(data.success){
   ...
   const addComment = async (e) => {
        e.preventDefault();
        try{
        const {data} = await axios.post(`${BASE_URL}/api/blog/add-comment`, {blog : id, name,
          content
        });
       if(data.success){
      ...
      <Navbar/>
       ...
       <div className='text-center mt-20 text-gray-600'>
        <p className='date-para py-4 font-medium'>Published on {Moment(data.createdAt).format('MMMM Do YYYY')}</p>
         ...
         mb-6 border text-sm font-medium text-primary author-para'>Piyush Raval</p>
       </div>
